...
import org.softwarefm.views.SoftwareFmContainer;
...
public class SoftwareFmActivator extends AbstractUIPlugin {
	...
	private ISelectedBindingManager<ITextSelection> makeSelectionBindingManager() {
		synchronized (lock) {
			if (selectionBindingManager == null) {
				...
				ISelectedBindingStrategy<ITextSelection, Expression> strategy = ISelectedBindingStrategy.Utils.eclipseSelectedBindingStrategy();
	...
	private IResourceGetter getResourceGetter() {
		return resourceGetter = resourceGetter == null ? IResourceGetter.Utils.resourceGetter(getClass(), "text") : resourceGetter;
	}
