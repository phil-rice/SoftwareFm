...
public class Detail implements IHasComposite {
	...
	public void setDetail(final Control control){
		...
		Size.removeOldResizeListener(content, detailResizeListener);
		
		detailResizeListener = new Listener() {
			@Override
			public void handleEvent(Event event) {
				// System.out.println("Resizing: " + detail.getClientArea());
				Size.setSizeToComputedSize(control, SWT.DEFAULT, content.getClientArea().height);
				content.layout();
			}
		};
		content.setContent(null);
		Size.setSizeToComputedSize(control, SWT.DEFAULT, content.getClientArea().height); // needed for scroll bar calculations
		System.out.println("detail: " + content.isDisposed() + ", control: " + control.isDisposed());
		content.setContent(control);
		Size.setSizeToComputedSize(control, SWT.DEFAULT, content.getClientArea().height); // needed again if the scroll bar popped in
