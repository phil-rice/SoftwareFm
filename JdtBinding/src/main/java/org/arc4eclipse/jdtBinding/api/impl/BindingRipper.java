			...
			if (from != null) {
				...
				if (javaElement != null) {
					...
					if (javaElement instanceof IMethod) {
						...
						return new BindingRipperResult(project, root, path, digestAsHexString, packageName, className, methodName);
					} else if (javaElement instanceof IClassFile) {
						String packageName = ((ITypeBinding) from).getPackage().getName();
						String className = javaElement.getElementName();
						return new BindingRipperResult(project, root, path, digestAsHexString, packageName, className, null);
					} else
						return new BindingRipperResult(project, root, path, digestAsHexString, "", "", "");
				}
			}
