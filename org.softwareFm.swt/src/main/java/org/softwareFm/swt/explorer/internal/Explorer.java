	...
	public Explorer(final IUserAndGroupsContainer container, final CardConfig cardConfig, final List<String> rootUrls, final IMasterDetailSocial masterDetailSocial, IPlayListGetter playListGetter, final ILoginStrategy loginStrategy, final IShowMyData showMyData, final IShowMyGroups showMyGroups, final IShowMyPeople showMyPeople, final IUserDataManager userDataManager, final Callable<Long> timeGetter) {
		...
		cardHolder = masterDetailSocial.createMaster(new IFunction1<Composite, ICardHolder>() {
			...
			public ICardHolder apply(Composite from) throws Exception {
				ICardHolder cardHolder = ICardHolder.Utils.cardHolderWithLayout(from, cardConfig, container, rootUrls, callbackToGotoUrlAndUpdateDetails);
				return cardHolder;
			}
		}, true);
		...
		masterDetailSocial.createAndShowMaster(TextInBorder.makeTextWithClick(SWT.WRAP | SWT.READ_ONLY, cardConfig, new Runnable() {
							...
							public UnrecognisedJar apply(Composite from) throws Exception {
								...
								UnrecognisedJar unrecognisedJar = new UnrecognisedJar(from, getCardConfig(), jarData, new ICallback<GroupidArtifactVersion>() {
									...
									public void process(GroupidArtifactVersion t) throws Exception {
										Map<String, Object> data = Maps.stringObjectMap(//
												JarAndPathConstants.groupId, t.groupId,//
												JarAndPathConstants.artifactId, t.artifactId,//
												JarAndPathConstants.version, t.version);
			...
			private Map<String, Object> guessDetailsForUnrecognisedJar(final File file) {
				...
				final Map<String, Object> startData = Maps.stringObjectMap(//
						JarAndPathConstants.groupId, file.getName().equals("rt.jar") ? "sun.jdk" : "Please specify the group id",//
						JarAndPathConstants.artifactId, guesser.guessArtifactName(file),//
						JarAndPathConstants.version, guesser.guessVersion(file));
				return startData;
			}
		...
		masterDetailSocial.createAndShowDetail(new IFunction1<Composite, IValueEditor>() {
			...
			public IValueEditor apply(Composite from) throws Exception {
				...
				return IValueEditor.Utils.cardEditorWithLayout(from, cardConfig, jarTitle, CollectionConstants.jarNotRecognisedCardType, "", startData, new ICardEditorCallback() {
					...
					public void ok(ICardData cardData) {
						masterDetailSocial.setDetail(null);
						String groupId = (String) cardData.data().get(JarAndPathConstants.groupId);
						String artifactId = (String) cardData.data().get(JarAndPathConstants.artifactId);
						String version = (String) cardData.data().get(JarAndPathConstants.version);
					...
					@Override
					public boolean canOk(Map<String, Object> data) {
						String groupId = Strings.nullSafeToString(data.get(JarAndPathConstants.groupId));
						String artifactId = Strings.nullSafeToString(data.get(JarAndPathConstants.artifactId));
						String version = Strings.nullSafeToString(data.get(JarAndPathConstants.version));
