...
abstract public class Container implements IContainerBuilder, IUserAndGroupsContainer {
	...
	public <API> ITransaction<Void> access(Class<API> clazz, final ICallback<API> callback) {
		...
		}, ICallback.Utils.<Void> noCallback(), readWriter);
	}
	...
	public <A1, A2> ITransaction<Void> access(Class<A1> clazz1, Class<A2> clazz2, final ICallback2<A1, A2> callback) {
		...
		}, ICallback.Utils.<Void> noCallback(), one, two);
	}
	...
	public <A1, A2, A3> ITransaction<Void> access(Class<A1> clazz1, Class<A2> clazz2, Class<A3> clazz3, final ICallback3<A1, A2, A3> callback) {
		...
		}, ICallback.Utils.<Void> noCallback(), one, two, three);

	}

	@Override
	public <Result, API> ITransaction<Result> access(Class<API> clazz, final IFunction1<API, Result> function, final ICallback<Result> resultCallback) {
		final API readWriter = getReadWriter(clazz);
	...
	@Override
	public <Result, A1, A2> ITransaction<Result> access(Class<A1> clazz1, Class<A2> clazz2, final IFunction2<A1, A2, Result> function, final ICallback<Result> resultCallback) {
	...
	@Override
	public <Result, A1, A2, A3> ITransaction<Result> access(Class<A1> clazz1, Class<A2> clazz2, Class<A3> clazz3, final IFunction3<A1, A2, A3, Result> function, final ICallback<Result> resultCallback) {
		final A1 one = getReader(clazz1);
		final A2 two = getReader(clazz2);
	...
	public <T> ITransaction<T> accessUserMembershipReader(IFunction2<IGroupsReader, IUserMembershipReader, T> function, ICallback<T> resultCallback) {
		return access(IGroupsReader.class, IUserMembershipReader.class, function, resultCallback);
	}
